<template>
  <a-card ref="cardRef" class="base-table-card-wrap relative" :bordered="false">
    ...
    >
      ...
      <a-table-column title="昵称" data-index="nickname" :width="150" ellipsis tooltip />
      ...
      <a-table-column title="电话" data-index="phone" :width="200" ellipsis tooltip />
      <a-table-column title="openid" data-index="open_id" :min-width="250" ellipsis tooltip />
      <a-table-column title="IP" data-index="ip" :width="150" ellipsis tooltip />
      <a-table-column
        title="更新时间"
        :width="160"
...
const tableData = ref<Type.BillUser[]>([])
const params: Partial<Type.QueryParams> = reactive({})
const editParams = ref<Partial<Type.BillUser>>({
  account: undefined
})
...
const methods = {
  ...
  fetchData: async () => {
    loading.value = true
    selected.value = []
    const { code, data } = await useBillUserApi.list({
  ...
  // 编辑数据
  handleEdit: (record: Type.BillUser) => {
  ...
  handleRestore: async (ids?: number[]) => {
    const { code, msg } = await useBillUserApi.restore(ids || selected.value)
    if (code === 200) {
      methods.fetchData()
      ResMsg(code, msg)
    }
  ...
  handleDelete: async (ids?: number[]) => {
    if (params.only_trashed) {
      const { code, msg } = await useBillUserApi.delete(ids || selected.value)
      if (code === 200) {
        methods.fetchData()
        ResMsg(code, msg)
      }
    } else {
      const { code, msg } = await useBillUserApi.remove(ids || selected.value)
      if (code === 200) {
        methods.fetchData()
        ResMsg(code, msg)
      }
